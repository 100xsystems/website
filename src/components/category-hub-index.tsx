...
import { getHubs, countHubResources } from '@/lib/knowledge-resources';
import { getLangIcon, getLangBg } from '@/lib/language-icons';

interface CategoryHubIndexProps {
  category: string;
  categoryLabel: string;
  description: string;
  /** Shown on the back-label chips under each course card. */
  hubLabel?: string;
}

/**
 * Generic knowledge-category hub index page body.
 * Used by knowledge categories that are real curriculum (databases,
 * frameworks, infrastructure, runtimes, data formats) but don't have a
 * bespoke hand-written index page yet.
 */
export function CategoryHubIndex({ category, categoryLabel, description, hubLabel = 'Topics' }: CategoryHubIndexProps) {
  const hubs = getHubs(category);
    ...
    <main className="min-h-screen bg-white">
      <section className="py-12 sm:py-28 bg-white">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          <div className="text-center mb-8 sm:mb-12">
            ...
            <p className="text-sm text-fg-secondary max-w-2xl mx-auto">{description}</p>
          </div>

          <div className="flex justify-center gap-6 sm:gap-10">
            <div className="text-center">
              <span className="block text-3xl font-extrabold text-fg">{hubs.length}</span>
              <span className="block text-[10px] font-bold uppercase tracking-widest text-fg-muted mt-1">{hubLabel}</span>
            </div>
            <div className="text-center">
      ...
      {hubs.length === 0 ? (
        <section className="py-20 text-center">
          <p className="text-sm text-fg-muted">{categoryLabel} courses land here after the next registry sync.</p>
        </section>
      ) : (
