/**
 * Shared Knowledge Lesson Page Component
 *
 * Consolidates ~928 lines of duplicated code across 6 route files into a single
 * reusable component + helpers. Each route page.tsx becomes a ~10-line wrapper.
 */
...
/** Load raw markdown for a lesson file from the knowledge-cache */
export function loadLessonMarkdown(category: string, hubSlug: string, lessonSlug: string): string | null {
  const filePath = path.resolve(
    process.cwd(),
    'public', 'knowledge-cache', category, hubSlug, `${lessonSlug}.md`
  );
  try {
...
/** Parse frontmatter + content from raw markdown */
export function parseLessonContent(raw: string): { content: string; frontmatter: Record<string, unknown> } {
  try {
...
/** Build Metadata for a lesson page (for generateMetadata) */
...
interface KnowledgeLessonPageProps {
  ...
  category: string;
  /** Display name for the hub (shown in back-link and title) */
  hubName: string;
  /** Slug of the hub (used to build lesson URLs) */
  hubSlug: string;
  /** Current lesson slug */
  lessonSlug: string;
  /** All lessons in this hub (for prev/next navigation) */
  lessons: LessonMeta[];
  /** URL path back to the hub detail page (e.g. /knowledge/principles/acid) */
  backUrl: string;
}

export async function KnowledgeLessonPage({
  category,
  hubName,
  hubSlug,
  lessonSlug,
  ...
  const raw = loadLessonMarkdown(category, hubSlug, lessonSlug);
      ...
      <div className="border-b border-border bg-surface-secondary">
        <div className="max-w-[900px] mx-auto px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {lesson.duration && (
              <span className="text-[9px] text-fg-muted">{lesson.duration}</span>
            )}
            {lesson.difficulty && (
