...
function loadLessonMarkdown(category: string, hubSlug: string, lessonSlug: string): string | null {
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const lesson = found.hub.lessons?.find((l) => l.slug === lessonSlug);
  return { title: `${lesson?.title || lessonSlug} — ${found.hub.name}` };
}
...
export default async function TechnologyLessonPage({ params }: Props) {
  ...
  const { hub, category } = found;
  const lesson = hub.lessons?.find((l) => l.slug === lessonSlug);
  if (!lesson) notFound();

  const raw = loadLessonMarkdown(category, slug, lessonSlug);
    ...
    <div className="min-h-screen bg-white">
      <div className="border-b border-border bg-surface-secondary">
        <div className="max-w-[900px] mx-auto px-6 py-3 flex items-center justify-between">
          <Link
            href={`/knowledge/technologies/${slug}`}
      ...
      <div className="max-w-[900px] mx-auto px-6 lg:px-12 py-12">
        ...
        <div className="mt-16 pt-8 border-t border-border flex items-center justify-between">
          {prevLesson ? (
            <Link href={`/knowledge/technologies/${slug}/${prevLesson.slug}`} className="group flex items-center gap-2 text-sm font-semibold text-accent hover:text-accent/80 transition-colors">
          ...
          {nextLesson ? (
            <Link href={`/knowledge/technologies/${slug}/${nextLesson.slug}`} className="group flex items-center gap-2 text-sm font-semibold text-accent hover:text-accent/80 transition-colors text-right">
              <div>
            ...
            <Link href={`/knowledge/technologies/${slug}`} className="flex items-center gap-2 text-sm font-semibold text-accent hover:text-accent/80 transition-colors">
