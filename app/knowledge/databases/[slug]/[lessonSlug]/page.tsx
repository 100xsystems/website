...
export async function generateStaticParams() {
  return getHubs('databases').flatMap((hub) =>
...
export default async function DatabasesLessonPage({ params }: Props) {
  const { slug, lessonSlug } = await params;
  const hub = getHub('databases', slug);
  if (!hub) notFound();
  const siblingHubs = getHubs('databases')
  ...
  return (
    <KnowledgeLessonPageWithProvider
      ...
      backUrlPrefix="knowledge/databases"
    />
  );
}
