...
function loadLessonMarkdown(hubSlug: string, lessonSlug: string): string | null {
  const filePath = path.resolve(
    process.cwd(),
    'public', 'knowledge-cache', 'tools', hubSlug, `${lessonSlug}.md`
  );
  try {
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: `${lesson?.title || lessonSlug} — ${item.title}` };
}
...
export default async function ToolLessonPage({ params }: Props) {
  ...
  const currentIndex = lessonList.findIndex((l) => l.slug === lessonSlug);
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-[900px] mx-auto px-6 lg:px-12 py-12">
        ...
        <div className="mt-16 pt-8 border-t border-border flex items-center justify-between">
          {prevLesson ? (
            <Link href={`/knowledge/tools/${slug}/${prevLesson.slug}`} className="group flex items-center gap-2 text-sm font-semibold text-accent hover:text-accent/80 transition-colors">
          ...
          {nextLesson ? (
            <Link href={`/knowledge/tools/${slug}/${nextLesson.slug}`} className="group flex items-center gap-2 text-sm font-semibold text-accent hover:text-accent/80 transition-colors text-right">
              <div>
            ...
            <Link href={`/knowledge/tools/${slug}`} className="flex items-center gap-2 text-sm font-semibold text-accent hover:text-accent/80 transition-colors">
