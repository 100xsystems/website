...
export async function generateStaticParams() {
  return getHubs('infrastructure').flatMap((hub) =>
...
export default async function InfrastructureLessonPage({ params }: Props) {
  const { slug, lessonSlug } = await params;
  const hub = getHub('infrastructure', slug);
  if (!hub) notFound();
  const siblingHubs = getHubs('infrastructure')
  ...
  return (
    <KnowledgeLessonPageWithProvider
      ...
      backUrlPrefix="knowledge/infrastructure"
    />
  );
}
