...
function loadLessonMarkdown(langSlug: string, lessonSlug: string): string | null {
  const filePath = path.resolve(
    process.cwd(),
    'public', 'knowledge-cache', 'languages', langSlug, `${lessonSlug}.md`
  );
  try {
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: `${lesson?.title || lessonSlug} — ${lang.name}` };
}
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-[900px] mx-auto px-6 lg:px-12 py-12">
        ...
        <div className="mt-16 pt-8 border-t border-border flex items-center justify-between">
          {prevLesson ? (
            <Link
              href={`/knowledge/languages/${slug}/${prevLesson.slug}`}
          ...
          {nextLesson ? (
            <Link
              href={`/knowledge/languages/${slug}/${nextLesson.slug}`}
